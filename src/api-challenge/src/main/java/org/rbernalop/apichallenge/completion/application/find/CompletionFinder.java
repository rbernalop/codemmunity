...
public class CompletionFinder extends UseCase {
    ...
    public FindUserCompletionsByChallengesResponse findUserCompletionsByChallengesIds(List<ChallengeId> challengeIds, UserUsername username) {
        List<Completion> completions = completionRepository.findByChallengeIdInAndUserUsername(challengeIds, username);
        return FindUserCompletionsByChallengesResponse.from(completions);
    }
}

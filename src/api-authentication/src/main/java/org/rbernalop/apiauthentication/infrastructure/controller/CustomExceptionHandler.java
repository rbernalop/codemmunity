...
public class CustomExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    public CustomError handleInvalidUserDataExceptionException(Exception ex) {
        return generateError(ex.getCause().getMessage(), HttpStatus.BAD_REQUEST);
    }

    @ResponseStatus(HttpStatus.CONFLICT)
    @ExceptionHandler(UserAlreadyExistsException.class)
    public CustomError handleUserAlreadyExistsException(Exception ex) {
        return generateError(ex.getCause().getMessage(), HttpStatus.CONFLICT);
    }
}
